...
function [chromosomes, Elite] = GA_IOD_lambert_nonrandom(Observation, Lambert_Type, use_3_pts, ACO)
rng("shuffle")
tic
...
a_T = 8378; % maximum semi-major axis for LEO (2000 km alt)
...
rho0_min = 300; % default value
rho0_max = 5500; % default value, max observable slant range for LEO (alt <= 2000), see max_slant_range.m
...
for i = 1:height(range_points)
    ...
    if a <= a_T && a >= 200+6378 && ecc >=0 && ecc <= ecc_max
...
while count_chromosomes < 1000
    ...
    if a <= a_T && a >= 200+6378 && ecc >=0 && ecc <= ecc_max && chromosomes.ranges(count_chromosomes+1,1) >= rho0_min && chromosomes.ranges(count_chromosomes+1,2) >= rho0_min
...
for i = 1:max_generations % begin GA
    ...
    while j <= 800
        ...
        if ecc <= ecc_max && a <= a_T && a >= 200+6378
    ...
    while counter <= 50
        ...
        if abs(chromosomes.ranges(counter+900, 1) - chromosomes.ranges(counter+900, 2)) < D && ecc <= ecc_max && a <= a_T && a >= 200+6378
